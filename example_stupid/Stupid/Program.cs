...
using File = funkylib.common.File;
...
namespace Stupid {
  ...
  public struct GameState {
    ...
    public static Option<GameState> parse(string playersCards, Suit trumpSuit) {
      ...
      return firstPlayer
        .zip(secondPlayer, (first, second) =>
          new GameState(first, second, CardPairs.Empty, trumpSuit)
        );
    } 
  }
  ...
  public static class Program {
    ...
    static Cards allCardsOnTable(CardPairs onTable) {
      ...
      var allCoveredCards = onTable.flatMap(pair => pair.cover.asEnumerable);
    ...
    static GameState defended (GameState state, Card defenseCard, CardPair toDefeat) {
      ...
      var newOnTable = state.onTable.Replace(
        toDefeat,
        new CardPair(toDefeat.placed, Option.Some(defenseCard))
      );
