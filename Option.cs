...
namespace funkylib {
  ...
  public static class OptionExt {
    ...
    public static Option<R> map<A, R>(this Option<A> @this, Func<A, R> func) {
      return @this.fold(
        () => Option.None,
        value => Option.Some(func(value))
      );
    }

    public static bool exist<A>(this Option<A> @this, Option<A> other, IEqualityComparer<A> comparer) =>
      @this.fold(
        () => false,
        left => other.fold(
          () => false,
          right => comparer.Equals(left, right)
        )
      );
    ...
    public static Option<R> apply<A, R>(this Option<Func<A, R>> @this, Option<A> option) => 
      @this.fold(
        () => Option.None,
        func => option.fold(
          () => Option.None,
          a => Option.Some(func(a))
        )
      );

    public static Option<Func<B, R>> apply<A, B, R>(this Option<Func<A, B, R>> @this, Option<A> option) => 
      apply(@this.map(F.curry), option);
